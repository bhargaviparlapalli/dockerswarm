...
# Creating 6 nodes 
echo "### Creating nodes ..."
for c in {1..6} ; do docker-machine create -d virtualbox node$c
done
...
for c in {2..3} ; do eval $(docker-machine env node$c)
    docker swarm join --token $master_token $leader_ip:2377
done

# Join node nodes
echo "### Joining node modes ..."
for c in {4..6} ; do eval $(docker-machine env node$c)
    docker swarm join --token $node_token $leader_ip:2377
done
